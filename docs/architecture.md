...
- Managed snippet markers: `AGENTS.md` contains a Taskplain-owned block bounded by `<!-- taskplain:start v0.2.0 -->` / `<!-- taskplain:end -->` (the version matches the CLI). Only the CLI updates this region, and refreshing it after upgrades is the recommended workflow even though the file sits outside `tasks/`.
